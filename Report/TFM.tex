...
%\addcontentsline{toc}{section}{Definition of the project}
...
As the idea of this document is to help people, who want to use or improve the package, to understand all the parts, the document is structures as Chapter 1 an introduction of the project with similar or complementary works. Chapter 2 explains some knowledge that should be known before to understand perfectly all the document. Chapter 3 is a brief summary of how it works the package and the most important if only wants to know how to use the package. Chapter 4 describes in detail all the parts of the package. In Chapter 5 are shown several experiments to see how precise is the package in any part. Chapter 6 explains the results from the previous chapter. Chapter 7 proposes several fields to improve the package. Finally, Chapter 8 explains the conclusions of the project.
...
On one hand, it is use an acknowledged software like ROS, so it can be easily implemented with other packages. On the other hand, it is also used the V4R software, which is from our institute (ACIN), due to is a novel software for the recognition.
...
\subsection{Grasping by Romeo with visual servoing}
...
The approach proposed, grasping with visual servoing, is a good initial approach, but it is not appropiate to achieve the main Romeo's goal of helping old people or people who has some kind of disability. Firstly, it only can accomplish the grasp with one arm, and in a complex environment probably in several cases it should be necessary to use both arms. Secondly, to know where is the hand it needs a specific position for the head and track it all the time, however, Romeo is supposed to have a good interaction with humans, but this features doesn't help it.

\subsection{MoveIt simple grasps}
...
\subsection{Romeo MoveIt Config}
...
\subsection{Romeo MoveIt Actions}
...
On the other hand, the real-time object pose tracking \cite{Prankl2013} \cite{Aldoma2013}, combines complementary interest points, for textured objects and for uniformly colored objects, in a common tracking framework which allows to handle a broad variety of objects regardless of their appearance and shape.
...
\texttt{camera\_reference\_frame\_id} & l\_wrist & Frame of the link used as reference, see Section \ref{sec:visual-recognition}. \\ \hline
...
\texttt{camera\_frame\_id} & camera\_rgb\_optica\_frame & Frame of the camera following the ROS Enhancement Proposals (REPs) as an optical camera frame \cite{Foote}.\\ \hline
\end{tabulary}
...
\begin{verbatim}
<rosparam command="load" ns="models/MODEL_NAME" 
file="$(find romeo_grasper)/data/models/MODEL_NAME/model_config.yaml" />
\end{verbatim}
...
In an environment where we can know the position of the camera respect to a link of the robot, use this way is a good option. Therefore we only need to set the configuration in the \texttt{romeo\_grasper\_xxxx.yaml} file. So with the \texttt{camera\_reference\_frame\_id} parameter we set the link used as reference and with \texttt{camera\_pose\_x}/\texttt{y}/\texttt{z} specify the coordinates where is the camera. However, we have to take in account that the orientation for this parameters is the \texttt{base\_link} orientation. That is done in that way to facilitate the user to take the coordinates.

Therefore, the process to calculate the position of the camera is, as described in Equation \eqref{eq:calc_camera_pose_pre-known}, taking position of the reference link in \texttt{base\_link} coordinates and adding the values of the parameters \texttt{camera\_pose\_x}$/$\texttt{y}$/$\texttt{z}. Finally, the transformation between \texttt{camera\_link} and \texttt{base\_link} is expressed like Equation \eqref{eq:trans_base_camera_pre-known}
...
%Experiment 2:
% Dir que es vol millorar el IK solver i per tant s'ha provat d'implementar IKFast, per això explicar tots els test que s'han arribat a fer. Tant els que son logics que van amb 6 DoF de torso a WristYaw com esta amb els grups especificat, com també fet des de torso fins a l_wrist posant un free joint (i s'ha provat amb tots els free joints). Posteriorment externament han canviat la nomenclatura i els origens de les joints i amb aquesta configuració s'ha tornat a provar. Finalment s'ha vist que nomes s'ha pogut aconseguir en el cas de Shoulder fins gripper (tant right com left) amb free joint al WristPitch. El que passa es que aquesta es inutil ja que primer es perd la joint ShoulderPitch ja que es troba entre torso i Shoulder i segon que una de les joints incloses es Hand que no dona moviment, per tant realment es tenen 5 DoF controlats que no permeten portar el gripper a determinada posició i orientació. Mencionar que aquesta solució es va aconseguir en els tests inicials abans de saber exactament que s'estava provant.
% Posar links de ses discussions que s'han obert per dur a terme això
\begin{itemize}
\item A way to improve the IK solver is to implement IKFast which is faster and more precise.
\item Firstly, explain how it is suppose to implement this. Like said before generating a C++ code. This is suppose to be automatically created running a specific program giving the Collada model.
\item Due to be a 7 DoF arm can exist several combinations. Explain all the tested combinations and which combinations should be the best.
\item After doing theses test, get some tips from Natalia Lyubova and Mikael Arguedas and Natalia rewrite the Romeo model to change the convention of the joints, it was all retest.
% Refer to discusion https://github.com/ros-aldebaran/romeo_robot/issues/10
\item It should be stated that every test last at least 1 or 2 hours. So a lot of time has been spent here.
\end{itemize}
...
In order to implement the IKFast on the Romeo robot, first is needed to generate the C++ code which solve the inverse kinematics. However, this is created from a Collada model file which is not found in the \texttt{romeo\_description} package. Then, this could be created, using the command below, from an URDF model of Romeo which is contained in the previous package.

\begin{verbatim}
rosrun collada_urdf urdf_to_collada romeo.urdf romeo_raw.dae
\end{verbatim}

After that, as recommended by the author of \cite{Diankov2010}, it is good to round the values of the Collada model to make easier to find aligned axes.
...
% Entre aquestes s'explicaria es fet de mirar d'aconseguir IKFast.
% Falta acabar de perfeccionar com seria la part del picking que ara mateix degut a que la cinematica inversa no funciona bé, no acaba d'aconseguir fer un picking.
\begin{itemize}
\item Reach to finally implement the IKFast so the inverse kinematic could work much better.
\item If it seems that IKFast is not possible there is the option of the IK use in \cite{Pas}. Is also a numerical solver, so probably it will be similar to the KDL, but only trying can be known.
\item After that it is supposed that the picking action will work well, because currently is not finding any picking due to the problems with the inverse kinematic.
\end{itemize}

\section{Improve use of V4R}
% Mirar de fer que la Toolbox de modelatje de V4R funciones amb RealSense ja que la camera RGB que es la que fa la major part de la feina té molt millor resolució o es podria treballar amb una Xtion que suposadament també funciona amb openNi.
% Fer que RealSense o en general qualsevol camera RGB-D pugui ser compatible amb la RTM-Toolbox, fent que faci com els altres packages i que cerqui per un tipus de topic determinat (en aquest cas el depth_registered/points i colcun més que s'ha de mirar a veure com va)
% També hi ha l'opcio de mirar a veure si es pot arribar a executar d'alguna forma openNi a dins el robot de tal manera que pugui arribar a publicar els topics d'openNi a ROS utilitzant la Xtion que du incorporada. Tot i que no es pugui veure els braços del tot be al menys els objecte que estiguin dins un rang determinat podran ser identificats en principi i ens estalviem l'haver de posicionar la camera que es una font d'error.
\begin{itemize}
\item Currently Kinect is giving some problems, like after some seconds or minutes the RGB camera starts to send black images. Therefore the V4R software cannot be used. Moreover, the OpenNi nodes after a while usually fail and they are automatically shutdown.
\item V4R works with SIFT on the RGB image, so having a good quality in the RGB image is important. If we compare Kinect with Xtion, this last one is better; and then comparing with RealSense is the best one.
\item Options to solve these problems are:
\begin{itemize}
\item Use RealSense for everything. However, to do that it needs to improve RTM-Toolbox to be able of using any RGB-D camera.
\item Use Xtion. Could be an external one, which during this work could not be used; or the one inside Romeo. With this last one, we have seen that you don't see the arms and only the objects in a specific and small range. Moreover, in order to solve the problem with dark images may be a possibility is to try to execute ROS with OpenNi inside the robot. This option will help to remove some error produced by the camera positioning.
\end{itemize}
\end{itemize}

\begin{figure}[h]
\begin{center}
\begin{subfigure}[r]{0.7\textwidth}
...
\end{subfigure}
\caption{Kinect problem, RGB camera sending black images}
\end{center}
\end{figure}

\section{Dependencies of packages}
% També dir tema de que es codi miri de ser cada vegada més independent del software V4R 
\begin{itemize}
\item Try to have the code the most independent of the V4R software. So then the grasping could be used in other fields or at least with others software.
\item Make the package as general as possible, in order to be able to be used for other robots. That could be made if the \texttt{romeo\_moveit\_actions} have the same functions of others robots, then it is supposed that it should not be very difficult.
\end{itemize}

\section{Object identification}
% Un posible treball futur podria ser en lloc de treballar amb l'object tracker, treballar amb l'identificador d'objectes de dins la base de dades del modelatjes que es facin, d'aquesta forma agafaria cadascun dels objectes, i per a cadascun tindria un offset determinat.
% Altre treball és el fet de trobar la forma d'extreure informació de l'objecte modelat i treure l'offset del centre geometric respecte l'origen de coordenades establert per V4R. D'aquesta forma no hi hauria error en la mesura d'on es troba el centre de l'objecte.
% La taula podria ser identificada per la camera, cosa que ja fan diferents softwares de per si, i si es necessari perque el software no du a terme la funció de posar-ho a la simulació, la informació que es treu del software es pot enviar al topic destinat a caracteritzar la taula en la simulació.
\begin{itemize}
\item Instead of working with the object tracker could be useful to use the object recognition. Recognise the objects which are in a database.
\begin{itemize}
\item That could be very useful
\begin{itemize}
\item Provides the centroid of the cluster which could be used to give to the package where it have to make the grasp
\item Provides a bounding box of the object which is how the object is represent in the simulation by now, but this would make to don't need to use a custom bounding box made by the user.
\item Provides a point cloud of the model transformed into camera coordinates. This information could be used for the AGILE grasp to grasp the desired object in the right way.
\item Identifying several objects at one could be used to implement a better way to use the visual positioning of the camera. Using a different identifier for every element which will help to find the camera position and orientation. 
\end{itemize}
\item This one is used with service, so it should call the service periodically to know every time where are the objects.
\end{itemize}
\end{itemize}

\section{Fix the Romeo model}
% També es pot comentar es tema de que el gripper ara mateix esta colocat en una posició que no es l'adecuada. Que s'hauria de corregir. Si es vol es poden utilitzar els valors del romeo_grasp_data.yaml pero aquests realment son presos de forma empirica.
\begin{itemize}
\item Currently the gripper in the Romeo model is posed in a wrong position. 
\item However, this pose is not used because for now is used the information in \texttt{romeo\_grasp\_data.yaml} of the \texttt{moveit\_simple\_grasps} which make an offset from the WristYaw\_link to pose the grasp. This information has been empirically set.
\item Should be good to make the model more standard and have the gripper in the right place and set the grasp from the gripper itself. 
\end{itemize}
\chapter{Conclusion}
